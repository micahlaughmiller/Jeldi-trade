...
from journal import format_close_card, format_day_table, format_entry_card
from position_manager import OpenSpread

ET = ZoneInfo("US/Eastern")


def _spread(qty: int = 2) -> OpenSpread:
    return OpenSpread(direction="BULLISH", setup="ORB", right="P", root="SPXW", expiration=date(2026, 9, 18),
                      short_strike=7510, long_strike=7505, width=5, qty=qty, entry_credit=3.10,
                      entry_time=datetime(2026, 9, 18, 10, 14, 32, tzinfo=ET), current_price=3.10, best_price=3.10)


def _row(qty: int = 2, remaining_total: int = 2, pnl: float = 60.0) -> dict:
    return {"date": "2026-09-18", "entry_time": datetime(2026, 9, 18, 10, 14, 32, tzinfo=ET),
...
def test_close_card_full_close():
    card = format_close_card(_row(), 1, day_pnl=60.0, closed_today=1, remaining=0)
    ...
    assert "P/L today       +$60.00   (1 trade closed)" in card
    assert "still open" not in card


def test_close_card_partial_runner_fill():
    row = _row(qty=1, remaining_total=2, pnl=30.0)
    row["position_closed"] = False
    card = format_close_card(row, 3, day_pnl=130.0, closed_today=2, remaining=1)
    assert " TRADE #3" in card
    assert "Bought back     1 of 2   (1 still open)" in card
    assert "P/L this fill   +$30.00" in card
    assert "P/L position    +$30.00 so far" in card
    assert "(2 trades closed)" in card


def test_day_table_lists_each_trade_and_total():
    table = format_day_table([_row(pnl=60.0), dict(_row(pnl=-40.0), exit_reason="STOP_LOSS")], 20.0)
    lines = table.splitlines()
    assert " TRADES TODAY" in lines[1]
    body = [l for l in lines if l.startswith(" 1 ") or l.startswith(" 2 ")]
    assert len(body) == 2 and "PROFIT_TARGET" in body[0] and "STOP_LOSS" in body[1]
    assert body[0].rstrip().endswith("+$60.00") and body[1].rstrip().endswith("-$40.00")
    assert lines[-2].rstrip().endswith("+$20.00")


def test_day_table_empty():
    assert "(no trades closed today)" in format_day_table([], 0.0)
