...
# Import Alpaca Connector from your project context
try:
...
class Scheduler45DTE:
    ...
    def __init__(self, alpaca_connector=None, logger_instance=None):
        self.alpaca = alpaca_connector or AlpacaConnector()
        self.logger = logger_instance or logger
        self.order_mgr = OrderManager(alpaca_connector=self.alpaca)
    ...
    def _load_existing_positions(self):
        """Loads existing positions from Alpaca, pairs legs into credit spreads, and fetches fill credits."""
        try:
            positions = self.alpaca.get_open_positions()
            
            if positions:
                log_msg = f"Loaded {len(positions)} raw position legs from Alpaca"
    ...
    def initial_check_and_scan(self):
        ...
        # Define market open / scan thresholds using dt_time safely
        ...
        # Check if we are past scan time and it's a weekday (Monday=0 to Friday=4)
        if now.weekday() < 5 and market_open_time <= current_time <= market_close_time:
            if current_time >= scan_target_time:
                ...
                # Call your scan method here when ready, e.g.:
...
if __name__ == "__main__":
    ...
    scheduler = Scheduler45DTE(alpaca_connector=alpaca)
    scheduler.run()